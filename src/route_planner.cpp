...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    // first, get the neighbors. This mutates the current_node.neighbors container
    current_node->FindNeighbors();

    for (auto neighbor: current_node->neighbors){
        neighbor->parent = current_node; // parent takes a pointer && current_node is a pointer
...
RouteModel::Node *RoutePlanner::NextNode() {
    sort( this->open_list.begin( ), this->open_list.end( ), [ ]( const RouteModel::Node *lhs, const RouteModel::Node *rhs )
    {
        return (lhs->g_value + lhs->h_value) > (rhs->g_value + rhs->h_value);
    });

    RouteModel::Node* least_node = this->open_list.back();
...
void RoutePlanner::AStarSearch() {
    ...
    while (not(this->open_list.empty())){
